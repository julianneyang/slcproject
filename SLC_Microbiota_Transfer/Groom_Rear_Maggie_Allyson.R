...
data<-read.csv("SLC_Microbiota_Transfer/Groom_Rear Analysis - Maggie_and_Allyson.csv", header=TRUE)
...
#Groom and Rear
a<-generate_boxplots(data, Genotype, Grooms,0,550) +
  ggtitle("Grooms") + 
  labs(x="", y="# Grooms") + 
...
b<-generate_boxplots(data, Genotype, Rears,0,550) +
  ggtitle("Rears") + 
  labs(x="", y="# Rears") + 
  theme(plot.title = element_text(hjust = 0.5))
b
...
c<-generate_boxplots(data, Genotype, Grooms,0,550) +
  ggtitle("Grooms") + 
  labs(x="", y="# Grooms") + 
  theme(plot.title = element_text(hjust = 0.5)) + 
  facet_wrap(~Sex)
...
d<-generate_boxplots(data, Genotype, Rears,0,550) +
  ggtitle("Rears") + 
  labs(x="", y="# Rears") + 
  theme(plot.title = element_text(hjust = 0.5))+
  facet_wrap(~Sex)
...
e<-generate_boxplots(data, Genotype, Grooms,0,550) +
  ggtitle("Grooms") + 
  labs(x="", y="# Grooms") + 
  theme(plot.title = element_text(hjust = 0.5)) + 
  facet_wrap(~Scorer)
...
data<-read.csv("SLC_Microbiota_Transfer/Groom_Rear Analysis - Maggie_and_Allyson.csv", header=TRUE)
data$Genotype <- data$SLC_Genotype
t.test(Grooms~Genotype,data)
wilcox.test(Grooms~Genotype,data)
