...
fit_data = Maaslin2(input_data=df_input_data, 
                    ...
                    output = paste0("PICRUST2_KO_Luminal_Colon_Maaslin2_Sex_Genotype_Sequencing_Run"), 
...
## WT vs MUT - no HET
data<-read.table("PICRUST2_KO_Luminal_Colon_Maaslin2_Sex_Genotype_Sequencing_Run/significant_results.tsv", header=TRUE)
data <- data %>% filter(qval <0.05)
data <- data %>% filter(metadata=="Genotype")
annotation <- read.delim("annotated_KO.tsv", row.names=1)
